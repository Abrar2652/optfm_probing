...
class TransConvwithPosembedding(nn.Module):
    ...
    def forward(self, x, y, edge_matrix):
        ...
        x = torch.cat((x, torch.sparse.mm(edge_matrix, y)), dim=1)
        print(x.shape)
        x = self.fcs[0](x)
...
class SGFormer_MIP(nn.Module):
    def __init__(self, in_channels_var, in_channels_cons, hidden_channels, out_channels,
                 trans_num_layers=1, trans_num_heads=1, trans_dropout=0.5, trans_use_bn=True, trans_use_residual=True, trans_use_weight=True, trans_use_act=True,
                 use_graph=True, graph_weight=0.8, aggregate='add'):
        super().__init__()
        ...
        self.trans_conv_var = TransConv(hidden_channels, hidden_channels, trans_num_layers, trans_num_heads, trans_dropout, trans_use_bn, trans_use_residual, trans_use_weight, trans_use_act)
        self.trans_conv_cons = TransConv(hidden_channels, hidden_channels, trans_num_layers, trans_num_heads, trans_dropout, trans_use_bn, trans_use_residual, trans_use_weight, trans_use_act)
        self.trans_conv_cross_constovar = TransConvwithPosembedding(hidden_channels, hidden_channels, trans_num_layers, trans_num_heads, trans_dropout, trans_use_bn, trans_use_residual, trans_use_weight, trans_use_act)
        self.trans_conv_cross_vartocons = TransConvwithPosembedding(hidden_channels, hidden_channels, trans_num_layers, trans_num_heads, trans_dropout, trans_use_bn, trans_use_residual, trans_use_weight, trans_use_act)
        # self.graph_conv = GraphConv(hidden_channels, hidden_channels, gnn_num_layers, gnn_dropout, gnn_use_bn, gnn_use_residual, gnn_use_weight, gnn_use_init, gnn_use_act)
        ...
        self.params0 = list(self.var_embedding.parameters())
        self.params0.extend(list(self.cons_embedding.parameters()))
        self.params1 = list(self.trans_conv_var.parameters())
        self.params1.extend(list(self.trans_conv_cons.parameters()))
        self.params1.extend(list(self.trans_conv_cross_constovar.parameters()))
        self.params1.extend(list(self.trans_conv_cross_vartocons.parameters()))
        self.params2 = list(self.GCN.parameters()) if self.GCN is not None else []
        self.params2.extend(list(self.fc.parameters()))
    ...
    def forward(self, cons_x, edge_index, edge_attr, var_x):
        ...
        var_emb = self.trans_conv_var(var_emb)
        cons_emb = self.trans_conv_cons(cons_emb)
        # if self.use_graph:
        #     var_emb, cons_emb = self.GCN(cons_emb, edge_index, edge_attr, var_emb)
        # else:
        #     pass
        
        attributes = torch.squeeze(torch.transpose(edge_attr, 0, 1), dim=0)
        
        sparse_matrix = torch.sparse_coo_tensor(edge_index, attributes, size=(cons_x.shape[0], var_x.shape[0]))
        reverse_sparse_matrix = sparse_matrix.t()
        
        var_emb = self.trans_conv_cross_constovar(var_emb, cons_emb, reverse_sparse_matrix)
        cons_emb = self.trans_conv_cross_vartocons(cons_emb, var_emb, sparse_matrix)
        
        padded_tensor = []
        for i in range(cons_emb.shape[0]):
            for j in range(var_emb.shape[0]):
                combined_row = torch.cat((cons_emb[i], var_emb[j]))
                padded_tensor.append(combined_row.unsqueeze(0))
        
        padded_tensor = torch.cat(padded_tensor, dim=0)
        
        x = self.fc(padded_tensor)
        return x, var_emb, cons_emb
    ...
    def reset_parameters(self):
        self.var_embedding.reset_parameters()
        self.cons_embedding.reset_parameters()
        self.trans_conv.reset_parameters()
        if self.use_graph:
