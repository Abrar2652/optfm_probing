...
**Virtual-global-node empirical check** (Lemma 5 in [paper/proof_sketch.md](paper/proof_sketch.md)): applying the `virtual_global_node` transform from [scripts/improvements.py](scripts/improvements.py) — which appends OPTFM's mean-pooled virtual constraint and variable nodes to every input, exactly as the pretraining pipeline does — leaves `cos_sim = 1.000000` and `exact_frac = 1.00` on every cell. The pretraining trick does not escape the 1-WL bound.
